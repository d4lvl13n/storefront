...
function FooterSkeleton() {
	return (
		<footer className="animate-skeleton-delayed bg-background text-muted-foreground opacity-0">
			<div className="mx-auto max-w-7xl px-4 pb-24 pt-12 sm:px-6 sm:pb-12 lg:px-8 lg:py-16">
				<div className="grid grid-cols-2 gap-8 md:grid-cols-4 lg:gap-12">
					{/* Brand column */}
					<div className="col-span-2 md:col-span-1">
						<div className="mb-4 h-7 w-24 animate-pulse rounded bg-secondary" />
						<div className="mt-4 space-y-2">
							<div className="h-4 w-full max-w-xs animate-pulse rounded bg-secondary" />
							<div className="h-4 w-3/4 max-w-xs animate-pulse rounded bg-secondary" />
						</div>
					</div>
					{/* Link columns */}
					{[1, 2, 3].map((i) => (
						<div key={i} className="hidden md:block">
							...
							<div className="space-y-3">
								{[1, 2, 3, 4].map((j) => (
									<div key={j} className="h-4 w-24 animate-pulse rounded bg-secondary" />
								))}
							</div>
						</div>
					))}
